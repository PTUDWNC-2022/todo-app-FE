...
const RightSideBar = ({ todoDetail }) => {
    ...
    function useOutsideAlerter(ref) {
        useEffect(() => {
            ...
            function handleClickOutside(event) {
                if (ref.current && !ref.current.contains(event.target) && !event.target.classList.contains('todo-item')) {
                    setExpand(false);
                } else {
                    setExpand(true);
                }
    ...
    return (
        <div className={`container w-25 ${!expand ? 'collapse-container' : ''}`} ref={containerRef}>
            <Card body>
                <Row>
